...
model_test_checkpoint = 'training_v2/'
...
valid_dataset_clean = filter_empty_examples(valid_dataset)

# extract useful inforamtionj
#trainset = train_data.map(process_qg_text)
validset = valid_dataset_clean.map(process_qg_text)
...
with open('inference_v1/inference.txt', 'w') as f:
    f.write("\n".join(predictions))
    
with open('inference_v1/ground_truth.txt', 'w') as f:
...
metrics_dict = compute_metrics(hypothesis='inference_v1/inference.txt', references=['inference_v1/ground_truth.txt'])
